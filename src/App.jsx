...
function App() {
  ...
  const adminId = JSON.parse(localStorage.getItem("userData"))?._id || "admin";

  return (
    <div style={{ display: "flex", background: "#E0F2FE", minHeight: "100vh" }}>
      {/* ⭐ Show sidebar only if not on login page */}
      {!hideSidebar && <Sidebar onToggle={(state) => setSidebarOpen(state)} />}
      ...
      >
        <ErrorBoundary>
          <Routes>
            ...
            <Route
              path="/tathastu-sessions"
              element={<AdminTathastuSessions />}
            />
            <Route
              path="/tathastu-sessions/:sessionId"
              element={<AdminTathastuReview />}
            />
            ...
            <Route
              path="/doccertificates"
              element={<DoctorCertificateManager />}
            />
